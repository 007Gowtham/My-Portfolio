...
export default function About() {
    ...
    const skills = dummyProfile.skills?.map((skill: { name: string }) => skill.name) || [];
    const about = dummyProfile.about || "";
    const internships = dummyProfile.internships?.map((i: { company: string; year: string; role: string; about: string }) => ({
        company: i.company,
        year: i.year,
        role: i.role,
        about: i.about
    })) || [];
    const ImageFile: string | null = dummyProfile.image || null;
            ...
            <div className="w-full max-w-7xl block md:hidden">
                ...
                <div className="bg-[#F6FBFF] shadow-2xl w-full max-w-sm mx-auto p-4 rounded-2xl mb-6">
                    <div className="relative box-shadow-custom w-full h-48 bg-black rounded-2xl mb-4">
                        <Image
                            src={ImageFile || "/about/profile.svg"}
                            alt="Profile Image"
                            fill
