    ...
    <div className="w-48 xs:w-52 sm:w-56 md:w-60 lg:w-64 xl:w-72 2xl:w-80 3xl:w-96 bg-[rgb(225,232,236)] border-r border-gray-300 flex flex-col">
      ...
      <div className="px-2 xs:px-3 sm:px-4 py-1.5 xs:py-2 border-b border-gray-300 bg-gray-200">
        <div className="flex items-center justify-between text-[#0E1C29] text-[10px] xs:text-xs uppercase tracking-wider">
          ...
          <div className="flex space-x-0.5 xs:space-x-1">
            <button className="hover:bg-gray-300 p-0.5 xs:p-1 rounded text-xs transition-colors">
              📁
            </button>
            <button className="hover:bg-gray-300 p-0.5 xs:p-1 rounded text-xs transition-colors">
              🔄
      ...
      <div className="px-1.5 xs:px-2 py-1 border-b border-gray-300 bg-gray-100">
        <div className="flex items-center text-[#0E1C29] text-xs xs:text-sm py-1">
          ...
          <span className="truncate text-[10px] xs:text-xs sm:text-sm">
      ...
      <div className="flex-1 overflow-y-auto">
        {fileItems.map((item, index) => (
          <div
            key={index}
            className={`flex items-center px-1.5 xs:px-2 py-0.5 xs:py-1 hover:bg-gray-300 cursor-pointer text-xs xs:text-sm text-[#0E1C29] transition-colors ${
              item.name === "Button.tsx"
                ? "bg-gray-300 border-l-2 border-blue-400"
                : ""
            }`}
          ...
          >
            {item.type === "folder" && (
              <div className="w-2.5 h-2.5 xs:w-3 xs:h-3 mr-1">
                {item.expanded ? (
                  <ChevronDown className="w-2.5 h-2.5 xs:w-3 xs:h-3" />
                ) : (
  ...
  const renderCodeWithSyntaxHighlight = (code: string) => {
    ...
    return lines.map((line, index) => (
      <div key={index} className="flex items-start">
        ...
        <span
          className="text-[#0E1C29] flex-1 text-[10px] xs:text-xs sm:text-sm whitespace-pre"
        >
          {line === "" ? "\u00A0" : line}
        </span>
      </div>
    ...
    <div className="w-60 xs:w-64 sm:w-72 md:w-80 lg:w-80 xl:w-96 2xl:w-[400px] 3xl:w-[450px] bg-[rgb(225,232,236)] border-l border-gray-300 flex flex-col">
      ...
      <div className="px-2 xs:px-3 sm:px-4 py-2 xs:py-3 border-b border-gray-300 bg-gray-100">
        <div className="flex items-center text-[#0E1C29] text-xs xs:text-sm">
          ...
          <span>main</span>
          <span className="ml-auto text-[10px] xs:text-xs text-gray-600">
            ↑2 ↓1
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="border-b border-gray-300">
          ...
          {stagedFiles.map((file, index) => (
            <div
              key={index}
              className="px-2 xs:px-3 sm:px-4 py-1.5 xs:py-2 hover:bg-gray-300 cursor-pointer flex items-center text-xs xs:text-sm transition-colors"
            >
              <span
                className={`w-3 h-3 xs:w-4 xs:h-4 rounded-sm text-[10px] xs:text-xs flex items-center justify-center mr-2 xs:mr-3 ${
                  file.status === "M"
              ...
              <span className="text-[#0E1C29] flex-1 truncate text-[10px] xs:text-xs sm:text-sm">
                {file.name}
              </span>
            </div>
          ))}
        ...
        <div>
          ...
          {commits.map((commit, index) => (
            <div
              key={index}
              className="px-2 xs:px-3 sm:px-4 py-2 xs:py-3 hover:bg-gray-300 cursor-pointer border-b border-gray-200 transition-colors"
            >
              <div className="flex items-start space-x-2 xs:space-x-3">
                <GitCommit className="w-3 h-3 xs:w-4 xs:h-4 text-gray-600 mt-0.5 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-xs xs:text-sm text-[#0E1C29] truncate">
                    {commit.message}
                  </div>
                  ...
                  <div className="text-[9px] xs:text-[10px] sm:text-xs text-green-600 mt-1">
                    {commit.changes}
                  </div>
                </div>
              </div>
      ...
      <div className="border-t border-gray-300 p-2 xs:p-3 sm:p-4 bg-gray-200">
        ...
        <div className="flex justify-between items-center mt-1 xs:mt-2">
          <div className="flex space-x-1 xs:space-x-2">
            <button className="text-[10px] xs:text-xs text-gray-600 hover:text-[#0E1C29] transition-colors">
              ✓ Commit
            </button>
            <button className="text-[10px] xs:text-xs text-gray-600 hover:text-[#0E1C29] transition-colors">
              ↑ Push
...
export const VSCodeWindow = () => {
  const [showMobilePanels, setShowMobilePanels] = useState({
    explorer: false,
    git: false,
  });
  ...
  return (
    <div className="relative w-full rounded-2xl bg-[rgb(225,232,236)]">
      {/* Background Effects */}
      <div className="absolute inset-0 bg-gradient-to-br from-gray-100/50 via-gray-200/50 to-gray-300/50" />
      <div className="absolute inset-0">
        <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-gray-300/30 rounded-full blur-3xl" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-gray-400/30 rounded-full blur-3xl" />
        <div className="absolute top-1/2 right-1/3 w-48 h-48 bg-gray-200/30 rounded-full blur-3xl" />
      </div>

      <div
        className="relative z-10 w-full h-[300px] xs:h-[350px] sm:h-[400px] md:h-[500px] lg:h-[600px] xl:h-[700px] 2xl:h-[800px] 3xl:h-[900px] max-w-7xl mx-auto bg-[rgb(225,232,236)] rounded-2xl overflow-hidden shadow-2xl ring-1 ring-gray-300 border border-gray-300"
        style={{
          boxShadow: `
      ...
      >
        ...
        <div className="bg-gray-200 border-b border-gray-300 px-2 xs:px-3 sm:px-4 py-1.5 xs:py-2 flex items-center justify-between">
          <div className="flex items-center space-x-2 xs:space-x-3">
            ...
            <div className="flex space-x-1 xs:space-x-2">
              <div className="w-2.5 h-2.5 xs:w-3 xs:h-3 bg-red-500 rounded-full shadow-lg ring-1 ring-gray-300" />
              <div className="w-2.5 h-2.5 xs:w-3 xs:h-3 bg-yellow-500 rounded-full shadow-lg ring-1 ring-gray-300" />
              <div className="w-2.5 h-2.5 xs:w-3 xs:h-3 bg-green-500 rounded-full shadow-lg ring-1 ring-gray-300" />
            </div>
          </div>
          ...
          <div className="flex-1 text-center text-[10px] xs:text-xs sm:text-sm text-[#0E1C29]">
            <span className="hidden sm:inline">
              Button.tsx — my-typescript-project
            </span>
          ...
          <div className="flex items-center space-x-1 md:hidden">
            ...
            <button
              className="p-1 hover:bg-gray-300 rounded text-[10px] xs:text-xs transition-colors"
              onClick={() =>
                setShowMobilePanels((prev) => ({ ...prev, git: !prev.git }))
              }
            >
              🔄
            </button>
            ...
            {showMobilePanels.git && (
              <div className="absolute inset-0 z-20 bg-black/20">
                ...
                <button
                  className="absolute top-2 right-2 p-2 bg-gray-200 rounded hover:bg-gray-300 transition-colors"
                  onClick={() =>
                    setShowMobilePanels((prev) => ({ ...prev, git: false }))
                  }
                >
                  <X className="w-4 h-4 text-[#0E1C29]" />
                </button>
